...
import { FileText, Download, ExternalLink, RefreshCw, AlertCircle, Loader2, Eye, EyeOff, X, Maximize2, Minimize2 } from 'lucide-react';
    ...
    const getPublicUrl = () => {
        ...
        return (
            <div className={isZenMode
                ? "fixed inset-0 z-[100] bg-background flex flex-col" // Zen Mode Styles
                ...
                <div className="flex items-center justify-between px-4 py-2 bg-white dark:bg-slate-950 border-b text-sm">
                    <div className="flex items-center gap-3">
                        <button
                            onClick={toggleZen}
                            className={`p-1.5 rounded transition-all flex items-center gap-2 ${isZenMode ? 'bg-primary/10 text-primary font-bold' : 'text-muted-foreground hover:bg-muted'}`}
                            title={isZenMode ? t('action.exitZen') : t('action.enterZen')}
                        >
                            {isZenMode ? <RefreshCw className="h-4 w-4 rotate-45" /> : <ExternalLink className="h-4 w-4" />} {/* Using generic icons if Eye/EyeOff are not imported, let's allow importing them? No, let's use what we have or Import them.*/}
                            {/* Actually I should import Eye/EyeOff. Let's fix imports first. */}
                            {isZenMode ? <span className="hidden sm:inline">Normal</span> : null}
                        </button>
