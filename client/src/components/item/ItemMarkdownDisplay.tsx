...
export function ItemMarkdownDisplay({ content, isSummary = false, className }: ItemMarkdownDisplayProps) {
    const defaultClasses = "w-full max-w-4xl bg-card p-8 rounded-lg prose dark:prose-invert max-w-none [&_ul]:list-disc [&_ul]:pl-6 [&_ul]:my-3 [&_ul]:space-y-1.5 [&_ul_ul]:list-[circle] [&_ul_ul]:pl-6 [&_ul_ul]:mt-1.5 [&_ol]:list-decimal [&_ol]:pl-6 [&_ol]:my-3 [&_ol]:space-y-1.5 [&_li]:my-1.5 [&_li>p]:my-0 [&_li>p]:inline-block";

    const summaryComponents = {
        h1: ({ children }: any) => <h1 className="text-3xl md:text-4xl font-extrabold text-blue-600 dark:text-blue-400 mb-6 border-b pb-4 mt-4">{children}</h1>,
        h2: ({ children }: any) => <h2 className="text-xl md:text-2xl font-bold text-blue-500 dark:text-blue-300 mt-8 mb-4">{children}</h2>,
        h3: ({ children }: any) => <h3 className="text-lg md:text-xl font-semibold text-blue-400 dark:text-blue-200 mt-6 mb-3">{children}</h3>,
        p: ({ children }: any) => <p className="text-base md:text-lg leading-relaxed text-slate-700 dark:text-slate-300 mb-3">{children}</p>,
        ul: ({ children }: any) => <ul className="list-disc pl-6 mb-4 space-y-2 [&_ul]:list-[circle] [&_ul]:pl-5 [&_ul]:mt-1.5 [&_ul]:space-y-1.5">{children}</ul>,
        ol: ({ children }: any) => <ol className="list-decimal pl-6 mb-4 space-y-2">{children}</ol>,
        li: ({ children }: any) => <li className="text-base md:text-lg text-slate-700 dark:text-slate-300 leading-relaxed my-1.5">{children}</li>,
        ...
        input: (props: any) => {
            ...
            return (
                <div className="flex items-center gap-2 my-1">
                    ...
                    <span className="text-base md:text-lg text-slate-700 dark:text-slate-300"></span>
                </div>
            )
        }
    };
    ...
    const formattedContent = isSummary ? formatSummaryMarkdown(rawContent) : rawContent;
