import { Plus, FolderPlus, Settings, Map as MapIcon, PanelLeftClose, PanelLeftOpen, BrainCircuit, LayoutGrid } from 'lucide-react'
    ...
    return (
        <div className={cn("flex flex-col h-full bg-card border-r transition-all duration-300", isCollapsed ? "w-20" : "w-64")}>
            ...
            <div className={cn("h-14 flex items-center border-b transition-all shrink-0", isCollapsed ? "justify-center px-2" : "justify-between px-4")}>
                {!isCollapsed && (
                    <Link to="/edu/dashboard" className="flex items-center gap-3 hover:opacity-80 transition-opacity overflow-hidden">
                        <img src={logoSrc} alt="EduTrack" className="h-8 w-8 object-contain" />
                        <span className="font-bold text-lg whitespace-nowrap">EduTrack</span>
                    </Link>
                )}

                {isCollapsed && (
                    <Link to="/edu/dashboard" className="shrink-0 hover:opacity-80 flex justify-center w-full">
                        <img src={logoSrc} alt="EduTrack" className="h-8 w-8 object-contain" />
                    </Link>
                )}
