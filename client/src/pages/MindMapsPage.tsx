...
export function MindMapsPage() {
    ...
    const handleDelete = (e: React.MouseEvent, id: string) => {
        e.stopPropagation();
        if (confirm(t('mindmap.delete.confirm'))) {
        ...
        <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-8">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-muted-foreground mt-1">{t('mindmap.subtitle')}</p>
                </div>
                <button
            ...
            {isLoading ? (
                ...
                <div className="text-center py-20 bg-muted/10 rounded-2xl border border-dashed">
                    ...
                    <h3 className="text-lg font-medium">{t('mindmap.empty.title')}</h3>
                    <p className="text-muted-foreground mb-6 max-w-md mx-auto">{t('mindmap.empty.desc')}</p>
                    <button
                        onClick={() => setIsGenerateModalOpen(true)}
                        className="px-4 py-2 bg-primary text-primary-foreground rounded-md hover:opacity-90 inline-flex items-center gap-2"
                    >
                        <Plus className="h-4 w-4" />
                        {t('mindmap.empty.action')}
                    </button>
                </div>
            ) : (
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center gap-4 text-xs text-muted-foreground">
                                        ...
                                        <span className="flex items-center gap-1">
                                            ...
                                            {(mm.sources?.noteIds?.length || 0) + (mm.sources?.fileNames?.length || 0)} {t('mindmap.sources')}
                                        </span>
                                    </div>
                                </div>
                                <button
